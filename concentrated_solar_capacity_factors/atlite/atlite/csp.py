...
import numpy as np
from dask.array import radians, sin, tan, sqrt
...
def calculate_dir_one_axis(ds, solar_position=None, altitude_threshold=3.75):
    """
    Calculate the direct irradiation on a one-axis tracking surface.

    Calculate the direct irradiation on a one-axis tracking surface based on the
    direct solar influx on a horizontal plane and the solar altitude.
    ...
    solar_position : xarray.Dataset (optional)
        ...
        Is calculated using atlite.pv.SolarPosition if omitted.
    altitude_threshold : float (default: 3.75 degrees)
        Threshold for solar altitude in degrees. See calculate_dni.
    """
    if solar_position is None:
        solar_position = SolarPosition(ds)
    ...
    # Calculate the direct irradiation on a one-axis tracking surface
    x_sun = sin(azimuth) * ds["influx_direct"] / tan(altitude)
    flux_dir_one_axis = sqrt(ds["influx_direct"]**2 + x_sun**2)
    dni = ds["influx_direct"] / sin(altitude)
    shortwave_downward = ds["influx_direct"]

    # Write out altitude, azimuth and flux_dir_one_axis to a file
    altitude.to_netcdf("altitude.nc")
    azimuth.to_netcdf("azimuth.nc")
    flux_dir_one_axis.to_netcdf("flux_dir_one_axis.nc")
    shortwave_downward.to_netcdf("downward_shortwave.nc")
    dni.to_netcdf("dni.nc")
    
    return flux_dir_one_axis